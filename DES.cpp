...
string Cipher(long long int msg, long long int key) {
	int f = 0;
	int keyr = (int)(key);
	int keyl = (int)(key >> 32);
	int c0 = 0, d0 = 0;
...
string encrypt_16b(long long int* msg_,long long int* key_)//16Byte msg 8Byte key
{
	return Cipher(sum_8(msg_), sum_8(key_)) + Cipher(sum_8(msg_ + 8), sum_8(key_));
}

string gen_challenge(string password, long long int* msga)//msga empty 16 byte
{
	long long int keya[8] = {0};
	srand(time(NULL));
	for (int i = 0; i < 16; i++)
		msga[i] = rand() % 256;
	for (int i = 0; i < password.size() && i < 8; i++)
		keya[i] = reverseBits(password[i]);
	return encrypt_16b(msga, keya);
}
